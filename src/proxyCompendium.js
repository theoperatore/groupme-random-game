...
const roll20Search =
  'https://app.roll20.net/compendium/compendium/globalsearch/dnd5e';

const search = async term =>
  fetch(`${roll20Search}?${qs.stringify({ terms: term })}`).then(response =>
    response.json()
  );

const getDef = async pagename =>
  fetch(`${roll20Base}/${qs.escape(pagename)}.json`).then(response =>
    response.json()
  );
